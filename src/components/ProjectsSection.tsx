...
const ProjectCard = ({ project, index, totalCards }: any) => {
  ...
  return (
    <div ref={cardRef} className="h-[85vh] flex items-center justify-center sticky top-24 md:top-32" style={{ marginTop: index === 0 ? 0 : `${index * 28}px` }}>
      <motion.div 
        style={{ scale }}
        className="w-full max-w-7xl mx-auto rounded-[40px] sm:rounded-[50px] md:rounded-[60px] border-2 border-[#D7E2EA] bg-[#0C0C0C] p-4 sm:p-6 md:p-8 flex flex-col gap-6 md:gap-10 shadow-2xl origin-top h-[90%] sm:h-auto"
      >
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6">
          <div className="flex items-center gap-4 sm:gap-8">
            <span className="font-black text-[#D7E2EA] text-[clamp(2.5rem,8vw,5rem)] leading-none">{project.num}</span>
            <div className="flex flex-col gap-1">
              <span className="font-light text-[#D7E2EA] opacity-60 uppercase text-xs md:text-sm tracking-widest">{project.category}</span>
              <h3 className="font-medium text-[#D7E2EA] text-xl md:text-3xl lg:text-4xl uppercase">{project.name}</h3>
            </div>
          </div>
          <LiveProjectButton className="hidden sm:block" />
        </div>

        <div className="flex flex-col md:flex-row gap-4 sm:gap-6 h-full flex-1">
          <div className="w-full md:w-[40%] flex flex-col gap-4 sm:gap-6">
            <img src={project.img1} loading="lazy" className="w-full h-[clamp(130px,16vw,230px)] rounded-[30px] sm:rounded-[40px] md:rounded-[50px] object-cover" alt="" />
            <img src={project.img2} loading="lazy" className="w-full h-[clamp(160px,22vw,340px)] rounded-[30px] sm:rounded-[40px] md:rounded-[50px] object-cover" alt="" />
          </div>
          <div className="w-full md:w-[60%] flex">
            <img src={project.img3} loading="lazy" className="w-full h-[300px] md:h-full rounded-[30px] sm:rounded-[40px] md:rounded-[50px] object-cover" alt="" />
          </div>
        </div>
      </motion.div>
    </div>
...
export const ProjectsSection: React.FC = () => {
  return (
    <section id="projects" className="bg-[#0C0C0C] rounded-t-[40px] sm:rounded-t-[50px] md:rounded-t-[60px] relative z-20 -mt-10 sm:-mt-12 md:-mt-14 pt-20 pb-32 px-5 sm:px-8 md:px-10">
